...
[^6]: -200 odds would mean you wager `$200` for a `$100` payout. For this project, -200 odds have been converted into 50, which represents the `$50` cash payout of winning this bet (disregarding the `$100` deposit). 
...
plot = preodds %>% 
  ...
  labs(title = "Figure 2: Need to redo this graph now", y = "Value",caption='Note: The higher the odds, the lower the championship likelihood') +
  theme_fivethirtyeight() + 
...
Table 3 shows the relationship between yearly league wide average attendance and the league wide gini measures for that season. Gini for team wins has no statistical significance but preseason gini does appear to have a strong statistical significance. The equation used for column 1 is shown below. 
